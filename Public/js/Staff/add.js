layui.use(['element', 'form', 'asuma'], function(){
  	...
  	var emailpos = $('.accountmsg').data('pos');
  	var defaultpwd = $('.accountmsg').data('pwd');
  	var emailmsg = $('.accountmsg').html();
  	
  	$('input[name=member_account]').on('input', function(){
  		var value = $(this).val();
  		if(defaultpwd == 'pre'){
  			if(!emailpos || emailpos == "" || emailpos == 'false'){
  		...
  		$('.accountmsg').html('登录账号为：'+value+emailpos+'，初始密码为：'+pwd);
  		if(value == ""){
  			$('.accountmsg').html(emailmsg);	
  		}
  	})
